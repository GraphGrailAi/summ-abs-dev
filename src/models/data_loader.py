...
def load_text(args, source_fp, target_fp, device):
    ...
    #n_lines = len(open(source_fp).read().replace('\r', ' ').replace('\n', ' ').replace('\r\n', ' ').split('\n '))
    n_lines = len(" ".join(open(source_fp).read().split()).split('\n'))
    if n_lines >= 10:
        #from nltk import tokenize
        #tokenize.sent_tokenize(open(source_fp).read())
        n_lines == 5

    def _process_src(raw):
        raw = raw.strip().lower().replace('\r', ' ').replace('\r\n', ' ')
